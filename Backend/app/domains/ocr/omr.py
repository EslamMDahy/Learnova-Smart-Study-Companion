...
def detect_bubbles(image: Any) -> list[Bubble]:
    ...
    The expensive part in the previous implementation was not HoughCircles
    itself; it was calculating fill ratios by creating one full-resolution mask
    per bubble on 150-DPI pages.  This version does all circle detection and
    fill-ratio scoring on a capped working image, then maps coordinates back to
    the original page.  That keeps OMR accurate enough for generated Learnova
    sheets and cuts multi-page scans from tens of seconds to roughly sub-second
    bubble extraction on typical servers.
    """
    try:
        import os
        import cv2
    ...
    try:
        ...
        max_work_longest = int(getattr(settings, "ocr_bubble_max_work_longest", os.getenv("OCR_BUBBLE_MAX_WORK_LONGEST", "1800")) or 1800)
    except Exception:
        max_work_longest = int(os.getenv("OCR_BUBBLE_MAX_WORK_LONGEST", "1800"))
    max_work_longest = max(650, min(1800, max_work_longest))
    ...
    # Score the inner fill on the original binary image, but only inside a
    # tiny ROI around each bubble. This preserves the old full-resolution OMR
    # accuracy without allocating a full-page mask for every bubble.
    ...
    for raw in np.round(circles[0, :]).astype("int"):
        wx, wy, wr = int(raw[0]), int(raw[1]), max(2, int(raw[2]))
        if wx - wr < 0 or wy - wr < 0 or wx + wr >= work_w or wy + wr >= work_h:
            continue

        x = int(round(float(wx) * inv_scale))
        y = int(round(float(wy) * inv_scale))
        r = max(3, int(round(float(wr) * inv_scale)))
        if x - r < 0 or y - r < 0 or x + r >= width or y + r >= height:
            continue

        inner = max(2, int(r * 0.62))
        x1, x2 = max(0, x - inner), min(width, x + inner + 1)
        y1, y2 = max(0, y - inner), min(height, y + inner + 1)
        roi = binary_full[y1:y2, x1:x2]
        if roi.size == 0:
            continue
        mask = np.zeros(roi.shape, dtype="uint8")
        cv2.circle(mask, (x - x1, y - y1), inner, 255, -1)
        dark_pixels = cv2.countNonZero(cv2.bitwise_and(roi, roi, mask=mask))
        area = max(1, cv2.countNonZero(mask))
        binary_ratio = float(dark_pixels) / float(area)
        gray_roi = gray_full[y1:y2, x1:x2]
        masked_values = gray_roi[mask > 0]
        darkness_ratio = 0.0 if masked_values.size == 0 else 1.0 - (float(masked_values.mean()) / 255.0)
        # Blue/black pen marks may not always cross the adaptive threshold in a
        # photographed scan, while the mean darkness still shows the selected
        # bubble clearly. Use the stronger of both signals.
        fill_ratio = max(binary_ratio, darkness_ratio)
